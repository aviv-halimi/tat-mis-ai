...
if ($_po_id) {
  $rs = $_PO->GetPO($_po_id);
  if ($r = getRow($rs)) {
    $meta_title = iif(strlen($r['po_name']), $r['po_name'], iif($r['po_type_id'] == 1, 'PO', 'CR') . ' Details') . ': ' . $r['po_number'] . ' (' . $r['po_status_name'] . ') - ' . $r['vendor_name'] . ' (' . getShortDate($r['date_created']) . ')';
    $po_code = $r['po_code'];
        ...
        <div class="row form-input-flat mb-2">
          <div class="col-sm-7">
            <div class="input-group">
              ...
              if ($_po_id and $_vendor_id) {
                ...
                $rv = getRs("SELECT vendor_id, concat(email, CASE WHEN NOT(ISNULL(secondary_email)) THEN concat(', ',secondary_email) ELSE '' END) email, name, scheduling_window, is_suspended FROM {$_Session->db}.vendor WHERE " . is_enabled() . " ORDER BY name");
          ...
          <div class="col-sm-5">
			...
			<div class="input-group mt-1">
              <div class="input-group-prepend">
                <div class="input-group-text">
                  <i class="fa fa-envelope mr-1"></i> Copy from PO #
                </div>
              </div>
              <input type="text" id="copy_po" name="copy_po" value="" class="form-control" placeholder="Optional ..." />
            </div>  
          </div>
...
if (true) { //sizeof($rs)) {
  ...
  if ($n = getRow($rn)) {
    array_push($_ns, yesNoFormat(1) . ' ' . $n['notification_type_name'] . ' sent at ' . getLongDate($n['date_created']) . '</b>');
  }
...
if ($t['po_status_id'] > 1 and strlen($_po_filename)) {
echo '
...
if ($_po_status_id == 1) {
  echo iif(!strlen($_description), '<button class="btn btn-default btn-po-description mt-3"><i class="ion-chatbubble-working mr-1"></i> Add Comments / Special Instructions</button>') . '
  ...
  <div class="panel panel-default mt-3 po-data po-description"' . iif(!strlen($_description), ' style="display:none;"') . '>
  <div class="panel-heading">
