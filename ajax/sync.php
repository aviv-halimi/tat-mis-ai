...
$run_queries = false; //true;
$update_keys = false; //true;
$_url = null;
$num_add = $num_update = $num_add_tbl = 0;

$ignored_fields = array('totalUsage', 'limitUsage', 'balance', 'paymentReceived', 'paidAmount', 'blazeTip', 'blazeFee', 'blazeTotalCharged', 'blazeCashBack363', 'memo', 'description', 'deviceName', 'address', 'city', 'licenseNumber', 'cartDiscountNotes');

$args = array(); //isset($_SERVER['argv'])?$_SERVER['argv']:null;
$table_index = 8; //isset($args[1])?$args[1]:-1;
$_start = -1; //isset($args[2])?$args[2]:-1;
$_store_id = 6; //isset($args[3])?$args[3]:null;
$_sync = 0; //isset($args[4])?$args[4]:0;
$_update = 1; //isset($args[5])?$args[5]:0;
$_id = null; //isset($args[6])?$args[6]:null;
...
$tables = array(
  ...
  array('name' => 'product', 'api' => 'store/inventory/products/dates', 'params' => 'startDate={ts}&skip={start}&limit=500'),
...
for ($_tStart = 0; $_tStart <= $_tStartMax; $_tStart+=100) {
if ($table_index == 0) $_start = $_tStart;
...
$num_add = $num_update = $num_add_tbl = 0;
  ...
  */
  ...
  if ($table) {
    ...
    $_sys_log_id = null; //dbPut($s['db'] . '._sys_log', array('tbl' => $fn, 'notes' => '(Auto) Start: ' . $start . iif($ts, ' | Ts: ' . $ts . ' | '  . $__params)));
        ...
        */
        ...
        if ($total) {
          if (($start + 100) < $total) {
            ...
            $start = 0;
            $ts = (time() - 300) * 1000; // 5 min earlier
          }
        }
        else {
          $start = 0;
        ...
        $update_params = json_encode(array('ts' => $ts, 'start' => $start, 'skip' => $skip, 'total' => $total, 'msg' => 'Completed ' . date('n/j/Y', time()) . ' ' . date('g:i a', time())));
      }
      //setRs("UPDATE {$s['db']}._sys_sync SET params = ? WHERE tbl = ?", array($update_params, $fn));
    }
    if ($fn == 'transaction') {
    ...
    //setRs("UPDATE {$s['db']}._sys_log SET duration = ?, notes = CONCAT(notes, ?) WHERE _sys_log_id = ?", array((time() - $run_start), iif($num_add_tbl, ' | Add: ' . $num_add_tbl) . iif($num_update, ' | Update: ' . $num_update), $_sys_log_id));
    ...
    if ($tbl == 'items' and $_k == 'productId') {
      ...
      if (!is_numeric($k) and !in_array($k, $ignored_fields) and (!sizeof($tbl_fields) || in_array($k, $tbl_fields)) and !is_array($v)) {
        ...
        */
        ...
        /*
        if (strtolower($tbl) == 'product' and strtolower($k) == 'name') {
          $__v = $v;
        }
        else {          
  ...
  if ($run_queries and $tbl and $insert_db) {
    ...
    if ($tbl) {
      ...
      if (!$__new) {
        ...
        if (in_array($tbl, array('product', 'member', 'transaction'))) {
          if ($___id) {       
      ...
      if ($update_keys and $primary_id) {
        ...
        if ($tbl == 'items') {
          if ($get_product) setRs("UPDATE {$s['db']}.`items` a INNER JOIN {$s['db']}.`product` b ON b.id = a.productId SET a.product_id = b.product_id WHERE a.items_id = {$primary_id}");
        }
...
//setRs("UPDATE setting_import SET table_index = (CASE WHEN table_index < 9 THEN (table_index + 1) ELSE 0 END) WHERE setting_import_id = 1");
