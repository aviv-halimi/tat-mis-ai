...
2. **Product type = the heading that immediately precedes the row**: In the PRODUCT TYPE column, a heading (e.g. "HALF OUNCE / 14G", "EIGHTHS / 3.5G", "SINGLE JOINTS / 1G") applies ONLY to the rows that appear at or below it, and only until the next product type heading. Do NOT assign a row to a product type that appears later on the page. For example, "Mad Honey #13" listed under "EIGHTHS / 3.5G" must get product_type "Eighths/3.5g"; "SINGLE JOINTS / 1G" applies only to the rows that come after that heading (e.g. Garlic Cocktail #7, SB36 #1), not to rows above it.
3. **Category is in the HEADINGS above the strain names**: The category is the column or section HEADING (e.g. "FLOWER", "SOLVENTLESS", "EDIBLES") — the text above the list of products, not in each row. Map that heading to category_id using the CATEGORY TRANSLATION RULES and the AVAILABLE CATEGORIES table below. The sub-headers (e.g. "HALF OUNCE/14G", "PERSY BADDER 1G") are the product_type, not the category. **Important:** If the product type or section heading is "SINGLE JOINTS / 1G" (or "SINGLE JOINTS/1G", "SINGLE JOINTS / 1 GRAM", "PREROLL", "JOINTS", "DOINKS", "PRE-ROLL"), the category must be Prerolls (Pre-Rolls), not Flowers.
4. **Brand is often at the top of the page and applies to everything on that page**: If the brand name appears once as a page or section heading (e.g. at the top of the menu), that brand applies to ALL products on that page/section. Use the same brand_id for every product on the page. Match the brand name to the AVAILABLE BRANDS table below and return its brand_id.
5. **Price — use the UNIT column**: The price for each product is in the column named "UNIT". Always take the price value from the UNIT column. Price must be a number for every row; never return null for price. If the same price applies to multiple rows (e.g. only the first row shows it), propagate that value to every row in that section.
6. **Genetic Filtering**: Ignore the "GENETICS" and "%" columns. Do not include their contents in the name field.
...
For the product_type field, provide the menu section or sub-header in proper/title case (e.g. "Half Ounce/14g", "Eighths/3.5g", "Persy Badder 1g", "LR Vape 1g All-In-One"). Use only the PRODUCT TYPE heading that immediately precedes that row (see rule 2); do not use a heading that appears later on the page. Do NOT return it in ALL CAPS. Set to null if not identifiable.
