...
$to_email = isset($params['boh_email']) ? trim($params['boh_email']) : '';
if ($to_email === '') {
	_json_err('This store has no BOH email configured.');
}
...
$note = 'Notify BOH: ' . $admin_name . ' sent an email notification to BOH.';
setRs("INSERT INTO file (re_tbl, re_id, admin_id, description, is_auto) VALUES ('po', ?, ?, ?, 0)", array($po_id, $admin_id, $note));

echo json_encode(array('success' => true, 'message' => 'Email sent to BOH.'));
exit();
