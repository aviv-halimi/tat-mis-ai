...
foreach($rs as $row) {
  ...
  if (isset($tbl['links'])) {
	  foreach($tbl['links'] as $_link) {
		  ...
		  $code = getIdCode($TableName, $row[$TableName . '_id']); //$row[$TableName . '_code'];
	...
	$dd .= '</div>';
	if (str_len($ModalUrl) || str_len($DetailsUrl) || $AllowDelete || $AllowEdit || $AllowAdd) array_push($record, $dd);
...
function formatField($row, $i, $type, $ref) {
	...
	*/
	...
	elseif (in_array($TableName, array('daily_discount', 'daily_discount_report')) && in_array($i, array('category_id', 'brand_id'))) {
		$t = str_replace('_id', '', $i);
		if ($ref) $t = $ref;
		$v = getDisplayName($t, $v, 'name', null, false, 'blaze1.');
	}
	...
	elseif ($i == 'blaze_ids' and $v) {
		$blaze_ids = isJson($v)?json_decode($v):array();
		if (sizeof($blaze_ids)) {
