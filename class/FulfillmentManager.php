...
class FulfillmentManager extends SessionManager {
    ...
    if ($t = getRow($rt)) {
      ...
      {$this->db}.category c RIGHT JOIN (
        {$this->db}.brand b RIGHT JOIN (
          ...
          ) ON ((_p.daily_sales = ceiling(p.daily_sales) AND ceiling(p.daily_sales) > 0) OR (_p.daily_sales = ceiling(p.daily_avg_sales) AND ceiling(p.daily_avg_sales) > 0) OR (_p.daily_sales = {$max_daily_sales} AND ceiling(p.daily_sales) > {$max_daily_sales} AND p.daily_avg_sales = 0) OR (_p.daily_sales = {$max_daily_sales} AND ceiling(p.daily_avg_sales) > {$max_daily_sales} AND p.daily_sales = 0) OR (_p.daily_sales = 1 AND ceiling(p.daily_avg_sales) = 0 AND p.daily_sales = 0))
        ) ON b.brand_id = p.brand_id
      ) ON c.category_id = p.category_id
      WHERE " . is_enabled('p') . " AND p.inv_1 > 0 AND COALESCE(_s.par_level, _b.par_level, _p.par_level, 0) > p.inv_2 AND NOT(p.name LIKE 'SAMPLE%') AND NOT (p.name LIKE 'PROMO%')"  . iif($_category_id, " AND p.category_id = ?") . iif($_brand_id, " AND p.brand_id = ?") . " ORDER BY " . implode(", ", $a_sql_order), $params);
      //
    }
    return $rs;
  }
  ...
  function SaveTransferReportAPI($_p) {
    ...
    if ($t = getRow($rt)) {
      ...
      $_rl = getRs("SELECT product_batch_location_type_id, inventoryId FROM {$this->db}.product_batch_location WHERE (product_batch_location_type_id = 1 OR product_batch_location_type_id = 2) AND is_primary = 1");
      foreach($_rl as $_l) {
  ...
  function TransferProduct($product_id, $productId, $qty, $from_inventoryId, $to_inventoryId, $updateInventory = false) {
    ...
    $this->SetEmployeeId();
	//$response = 'from: ' . $from_inventoryId . ' | to: ' . $to_inventoryId . ' | ProductId: ' . $productId . ' | Qty: ' . $qty;
    $transferLogs = array();
	...
	if (!isset($from_inventoryId) || !isset($to_inventoryId)) {
		$response = 'Primary Inventory Locations are not set.  Please set them by going to Fulfillment Transfer > Meta Data > Inventory Locations';
	}
    if (!str_len($response)) {
      $api_post = array(
      ...
      $json = postApi2('store/batches/transferInventory', $this->api_url, $this->auth_code, $this->partner_key, $api_post);
      if (isJson($json)) {
        ...
        */
        if (isset($a_json['transferNo'])) {
          ...
          if ($updateInventory) {
            $this->FastUpdateInventory($product_id, $productId);
          }
        }
        else {
  ...
  function UpdateLatestProductInventory($_p) {
    ...
    foreach($rs as $r) {
      $this->FastUpdateInventory($r['product_id'], $r['id']);
  ...
  function UpdateInventory($product_id, $productId) {
		 $rs = getRs("SELECT store_name, store_id, db, api_url, auth_code, partner_key, params FROM store WHERE " . is_enabled() . iif($store_id, 		" AND store_id = {$store_id}") . " ORDER BY store_id DESC");
	...
	LEFT JOIN (
	  ...
	  WHERE " . is_active('p') . " AND p.active = '1' AND p.deleted = '' AND p.is_batch_updated = 0 " . iif($product_id, ' AND p.product_id = ' . $product_id) . "
	  GROUP BY p.product_id, p.id, p.sku, p.name, p.category_id, p.brand_id, p.vendor_id, p.unitPrice,id.DaysInInventory");
	  ...
	  $ra = getRs("SELECT category_id, brand_id, IFNULL(SUM(valid_sales)/nullif(SUM(sale_days),0),0) AS daily_sales FROM {$s['db']}.product_sale WHERE category_id = ? AND brand_id = ? GROUP BY category_id, brand_id", array($category_id, $brand_id));
	  foreach($ra as $a) {
			  ...
			  if ($pBatch != $b['batchId']) {
				...
				if (isset($batches['values'])) {
					foreach($batches['values'] as $batch) {
						if (isset($batch['costPerUnit']) and is_numeric($batch['costPerUnit']) and $batch['costPerUnit'] > 0.01) {
							$costPerUnit = numFormat($batch['costPerUnit']);
							if (is_numeric($b['batchPurchaseDate']) AND isset($product_id) AND $pBatch != $b['batchId']) $set = setRs("UPDATE theartisttree.po_product pp INNER JOIN theartisttree.po p ON p.po_id = pp.po_id SET pp.blaze_cogs = {$costPerUnit} WHERE (p.date_received >= FROM_UNIXTIME(({$b['batchPurchaseDate']} / 1000)- (2*86410) ) AND p.date_received <= FROM_UNIXTIME(({$b['batchPurchaseDate']} / 1000) + (2*86410))) AND pp.received_qty > 0 AND p.is_active = 1 AND p.is_enabled = 1 AND pp.is_active = 1 AND pp.is_enabled = 1 AND p.po_status_id > 3 AND pp.product_id = {$product_id} AND p.store_id = {$t_store_id}");
						}
					}
			  }
			  $pBatch = $b['batchId'];

			  dbPut($s['db'] . '.product_batch', array('product_id' => $product_id, 'batchId' => $b['batchId'], 'inventoryId' => $b['inventoryId'], 'qty' => $b['quantity'], 'costPerUnit' => $costPerUnit, 'created' => $b['created'], 'modified' => $b['modified'], 'batchPurchaseDate' => $b['batchPurchaseDate']));
		  }
		}
	  }
	...
	function FastUpdateInventory($product_id = null, $productId = null) {
