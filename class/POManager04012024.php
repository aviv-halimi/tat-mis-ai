...
class POManager extends SessionManager {
    ...
    function GetSavedPOProducts($po_id, $_brand_id = null, $_category_id = null, $_date_last_purchased = null, $_disaggregate_ids = array(), $_sort_by = null, $_po_product_id = null) {
	  ...
	  ORDER BY COALESCE(s.sort, t.po_product_id), " . implode(", ", $a_sql_order), $params);
    }
    ...
    return array($total, $ret);
      ...
      function POProgress($po_id) {
        ...
        if ($t = getRow($rt)) {
          ...
          foreach($rd_pt as $d) {
            $foot .= '
            <tr><td class="p-0" style="text-indent:30px"><div>' . iif($d['discount'], iif(strlen($d['po_discount_name']), $d['po_discount_name'], 'Pre tax Discount') . iif($d['discount_rate'], ' (-' . (float)$d['discount_rate'] . '%)'), 'Pre tax Discount') . '</div></td><td class="p-0 text-right">-' . currency_format($d['discount']) . '</td></tr>';
            $pt_discount += $d['discount'];
