...
class SessionManager {
  ...
  var $store_id = null;
	...
	var $settings = array('blocked-ips' => '');
	var $site_ids = '[]';
	var $module_ids = '[]';
  var $admin_settings = array();
	...
	function InitSession($check_store = true) {
		...
		if (isset($_SESSION['admin_id']) && is_numeric($_SESSION['admin_id']) ) {
			$rs = getRs("SELECT a.*, g.module_ids, CONCAT('/media/admin/sm/', CASE WHEN LENGTH(a.image) THEN a.image ELSE 'no.png' END) AS image_url, s.db, s.api_url, s.auth_code, s.partner_key FROM store s RIGHT JOIN (admin a LEFT JOIN admin_group g ON g.admin_group_id = a.admin_group_id AND " . is_enabled('g') . ") ON s.store_id = a.store_id WHERE (a.is_superadmin = 1 OR ((a.date_start IS NULL OR a.date_start <= CURDATE()) AND (a.date_end IS NULL OR a.date_start >= CURDATE()))) AND " . is_enabled('a') . " AND a.admin_id = ?", array($_SESSION['admin_id']));
			if ($row = getRow($rs)) {
	...
	*************************************************************************************************************************************/
	...
	function ForgotPassword($_p) {
		...
		if (str_len($email) == 0) {
			...
			$rs = getRs("SELECT a.admin_id, a.admin_name, a.first_name, a.last_name, a.email, a.is_enabled, CASE WHEN (a.is_superadmin = 1 OR ((a.date_start IS NULL OR a.date_start <= CURDATE()) AND (a.date_end IS NULL OR a.date_start >= CURDATE()))) THEN 0 ELSE 1 END AS is_expired FROM admin a WHERE a.site_id = ? AND a.email = ? AND " . is_active('a'), array($this->site_id, $email));
			if ($row = getRow($rs)) {
        if ($row['is_expired']) {
          $response = 'Sorry ' . $row['first_name'] . ', your credentials are no longer valid. Please contact system administrator for more information.';
        }
    ...
    $remember = 1;
		...
		if (!str_len($response)) {
			$rs = getRs("SELECT admin_id, email, is_enabled FROM admin WHERE site_id = ? AND password_token = ? AND " . is_active(), array($this->site_id, $password_token));
			if ($row = getRow($rs)) {
				if ($row['is_enabled'] == 1) {
	...
	*************************************************************************************************************************************/
	...
	function SendNotification($email_notification_code, $placeholders, $attachment = '', $attachment_name = '') {
		...
		if ($row = getRow($rs)) {
			...
			$arr = sendEmail($placeholders['to_name'], $placeholders['to_email'], $subject, $message, $row['bcc'], $attachment, $attachment_name);
			$success = $arr['success'];
