...
class POManager extends SessionManager {
    ...
    function AddPOProducts($po_id, $rs, $po_reorder_type_id = null, $is_editable = 0, $is_non_conforming = 0, $oPrice = null) {
      ...
      foreach($rs as $r) {
		...
		*/
          ...
          dbPut('po_product', array('po_id' => $po_id, 'product_id' => $r['product_id'], 'product_name' => $r['product_name'], 'category_id' => $r['category_id'], 'brand_id' => $r['brand_id'], 'par_level' => $r['par_level'], 'reorder_level' => $r['reorder_level'], 'inv_1' => $r['inv_1'], 'inv_2' => $r['inv_2'], 'on_order_qty' => $r['on_order_qty'], 'daily_sales' => $daily_sales, 'cost' => $tcost, 'weight_per_unit' => nicefy($r['weightPerUnit']), 'cannabis_type' => nicefy($r['cannabisType']), 'flower_type' => nicefy($r['flowerType']), 'suggested_qty' => $suggested_qty, 'is_editable' => $is_editable, 'is_non_conforming' => $is_non_conforming, 'is_tax' => ((strtolower($r['cannabisType']) != 'non_cannabis')?1:0), 'date_last_purchased' => $r['date_last_purchased']));
        }
      }
      return $num_products;
    }
    ...
    function GetSavedPOProducts($po_id, $_brand_id = null, $_category_id = null, $_date_last_purchased = null, $_disaggregate_ids = array(), $_sort_by = null, $_po_product_id = null) {
      ...
      return getRs("SELECT po.po_code, po.po_status_id, t.po_product_id, t.po_product_code, COALESCE(c.category_id,c2.category_id,'0') as category_id, COALESCE(c.name,c2.name,'(No Category Listed)') AS category_name, ifnull(b.brand_id,0) as brand_id, ifnull(b.name,'(No Brand Listed)') AS brand_name, b.is_suspended, s.product_id, COALESCE(s.name, t.po_product_name) AS product_name, COALESCE(s.sku, 'NEW*') AS sku, s.id, t.inv_1, t.inv_2, t.par_level, t.reorder_level, ROUND(t.daily_sales,1) as daily_sales, t.on_order_qty, t.suggested_qty, t.order_qty, t.received_qty, t.price, t.paid, t.cost, t.is_editable, t.is_non_conforming, t.weight_per_unit, t.cannabis_type, t.flower_type, t.is_tax, (case when po.po_status_id = 1 then t.is_editable else 0 end) as sort2, case when po.po_status_id = 1 and t.is_editable then t.po_product_id else COALESCE(s.name, t.po_product_name) end as sort3 FROM 
      ({$this->db}.category c RIGHT JOIN (
        {$this->db}.brand b RIGHT JOIN (
  ...
  function SavePOStatus($_p) {
    ...
    if (!str_len($response)) {
      $rs = getRs("SELECT po_id, vendor_id, po_type_id, po_status_id, num_products, num_orders, num_received, po_filename, invoice_filename, date_received, invoice_number, ifnull(discount,0), ifnull(r_discount,0), ifnull(at_discount,0), ifnull(r_at_discount,0) FROM po WHERE " . is_enabled() . " AND po_code = ?", array($po_code));
      if ($r = getRow($rs)) {
        $po_id = $r['po_id'];
  ...
  function NabisSummary($nabis_id = null) {
	...
	$rdp = getRs("SELECT *, SUM(quantity) as totalQty FROM nabis_product WHERE quantity < 5 AND pricePerUnit = .01 AND nabis_id = {$nabis_id} and is_enabled = 1 and is_active = 1");
	if ($rd = getRow($rdp)) {
		if ($rd['totalQty'] > 0) {
		...
		$clear = getRs("UPDATE nabis_product SET is_active = 0 WHERE quantity < 5 AND pricePerUnit = .01 AND nabis_id = {$nabis_id} and is_enabled = 1 and is_active = 1");
		}
	}
  ...
  function NabisPO($nabis_id) {
    ...
    $email = '';
    $po_code = null;
    $rn = getRs("SELECT * FROM nabis WHERE nabis_id = ?", array($nabis_id));
    if ($n = getRow($rn)) {
  ...
  function NabisDiscount($nabis_id, $footer = true) {
    ...
    if ($r = getRow($rs)) {
      $rd = getRs("SELECT brand_discount_id, brand_discount_name, brand_id, discount_rate, is_after_tax FROM {$this->db}.brand_discount WHERE " . is_enabled() . " AND brand_id IS NOT NULL AND COALESCE(discount_rate, 0) > 0 ORDER BY brand_discount_id");
      foreach($rd as $d) {
        $rp = getRs("SELECT SUM(quantity * price) AS subtotal FROM nabis_product WHERE " . is_enabled() . " AND nabis_id = ? AND brand_id = ?", array($nabis_id, $d['brand_id']));
        $po_discount_code = null;
        if ($p = getRow($rp)) {
          if ($p['subtotal']) {
