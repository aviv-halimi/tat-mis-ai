...
/**
 * Fill a single worksheet with Trial Balance content (for multi-sheet workbooks).
 ...
 * @param array $style_title (etc.)
 * @return void
 */
function qbo_tb_fill_sheet_from_parsed(
    $sheet,
    $columns, $rows, $header_row1, $header_row2, $store_name, $start_date, $end_date,
 ...
 */
function qbo_tb_sheet_title($store_name, &$used_titles = array()) {
    $safe = preg_replace('/[:\\\\\/\?\*\[\]]/', '', $store_name);
    ...
    $base = $safe;
    $n = 2;
    while (isset($used_titles[$safe])) {
        $suffix = ' (' . $n . ')';
        $safe = mb_substr($base, 0, 31 - strlen($suffix)) . $suffix;
        $n++;
    }
    $used_titles[$safe] = true;
    return $safe;
}

/**
 * Write Trial Balance Excel from parsed report (single-sheet file).
 * @param array $columns
 * @param array $rows
 * @param array|null $header_row1
 * @param array|null $header_row2
