...
$_store_id = getVarNum('_store_id', 1);
...
$tables = array(
  ...
  array('name' => 'transaction', 'api' => 'transactions/65918b7ad7972078ad3eb84e', 'params' => 'skip={start}'),
...
function dumpArray($s, $a, $fn, $tbl = null, $parents = array(), $parent_id = null) {
  ...
  foreach($a as $k => $v) {
    ...
    if ($tbl == 'transaction' and $_k == 'memberId') {
      ...
      $get_member = (str_len($v))?true:false;
    ...
    if ($tbl == 'transaction' and $_k == 'sellerId') {
      ...
      $get_employee = (str_len($v))?true:false;
      if (tableExists($s['db'], 'transaction') and tableExists($s['db'], 'member')) {
    ...
    if ($tbl == 'items' and $_k == 'productId') {
      ...
      $get_product = (str_len($v))?true:false;
      ...
      if (sizeof($v)) {
        ...
        if ($_k == 'tags') {
          if (is_array($v) and sizeof($v)) {
            ...
            $params['tags'] = (str_len($v) > 50)?substr($v, 0, 49):$v;
          }
        }
        ...
        //////////////////////// transaction /////////////////
          ...
          if (is_array($v) and sizeof($v)) {
            ...
            $params['orderTags'] = (str_len($v) > 50)?substr($v, 0, 49):$v;
          }
        }
        ...
        //////////////////////// vendor /////////////////
          ...
          if (isset($v['address'])) {
            ...
            $params["`address`"] = (str_len($v['address']) > 255)?substr($v['address'], 0, 254):$v;
          }
      ...
      if (!is_numeric($k) and !in_array($k, $ignored_fields) and (!sizeof($tbl_fields) || in_array($k, $tbl_fields)) and !is_array($v)) {
        array_push($fields, $k);
        $params["`" . $k . "`"] = (str_len($v) > 50)?substr($v, 0, 49):$v;
      }
    }
    $ret .= '</li>';
  }
