...
<div class="row">
    ...
    <div class="col-md-6">
                    ...
                    <div class="m-0 panel panel-inverse">
			            <div class="panel-heading">
			                ...
			                <h4 class="panel-title">Sync Activities</h4>
			            </div>
			            <div class="panel-body" data-scrollbar="true" data-height="270px">
							<ul class="feeds">
                                <?php
                                $rl = getRs("SELECT * FROM {$_Session->db}._sys_log ORDER BY _sys_log_id DESC LIMIT 10");
                                $i = 0;
                                foreach($rl as $l) {
                                    $i++;
                                echo '
								<li>
									<a href="javascript:;">
										' . iif($l['duration'], '<div class="icon bg-success-light"><i class="fa fa-check"></i></div>', iif($i > 1, '<div class="icon bg-danger-light"><i class="fa fa-times"></i></div>', '<div class="icon bg-warning-light"><i class="fa fa-info-circle"></i></div>')) . '
										<div class="time">' . getTimeDiff2($l['date_created']) . ' ago</div>
										<div class="desc">' . ucwords($l['tbl']) . '' . iif($l['duration'], '<span class="label label-success m-l-5">Completed' . iif($l['duration'], ' in ' . formatTime($l['duration'])) . '</span>', iif($i > 1, '<span class="label label-danger m-l-5">Incomplete</span>', '<span class="label label-inverse m-l-5">Running</span>')) . '</div>
									</a>
								</li>';
                                }
                                ?>
