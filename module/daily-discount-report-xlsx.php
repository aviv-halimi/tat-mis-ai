...
if ($daily_discount_report_brand_code) {   
    $rd = getRs("SELECT r.*, b.name AS brand_name, b.brand_id, rb.daily_discount_report_brand_id FROM blaze1.brand b INNER JOIN (daily_discount_report_brand rb INNER JOIN daily_discount_report r ON rb.daily_discount_report_id = r.daily_discount_report_id) ON rb.brand_id = b.brand_id WHERE rb.daily_discount_report_brand_code = ?", $daily_discount_report_brand_code);
}
else {
